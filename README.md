...
在标注页面新增了文档内容编辑模式切换功能，允许用户在两种工作模式之间切换：

- **只读模式**（默认）：左侧文档内容为只读状态，只能通过右侧表单进行标注
- **编辑模式**：左侧文档内容变为可编辑的JSON编辑器，可直接修改文档数据

#### 功能特性：

1. **模式切换按钮**：位于顶部工具栏
   - 只读模式：显示"开启编辑"按钮，点击切换到编辑模式
   - 编辑模式：显示"切换只读"按钮（绿色高亮），点击切换到只读模式

2. **双重编辑方式**：
   - **表单编辑**：右侧标注表单始终可用，支持结构化的字段编辑
   - **直接编辑**：编辑模式下，左侧Monaco编辑器支持直接编辑JSON内容

3. **数据同步机制**：
   - 表单修改会实时反映到左侧JSON显示
   - JSON编辑器的修改会同步到表单数据
   - 两种编辑方式的修改都会标记为"已修改"状态

4. **数据完整性保证**：
   - 所有修改（表单或JSON编辑器）都会触发相同的数据校验
   - JSON格式错误时不会影响数据，但会在控制台给出警告
   - 保存时统一传输到后端进行完整的数据校验

#### 使用场景：

- **表单标注**：使用右侧结构化表单，适合标准化的数据录入
- **快速编辑**：使用JSON编辑器，适合熟悉数据结构的用户快速修改
- **批量调整**：在JSON编辑器中进行复制粘贴等批量操作
- **调试验证**：查看完整的JSON结构，便于理解数据关系

#### 技术实现：

- 使用Monaco Editor提供专业的JSON编辑体验
- 双向数据绑定确保表单和编辑器内容同步
- 防抖机制避免频繁的数据更新
- JSON解析错误时的优雅降级处理
