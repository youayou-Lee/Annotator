...
from app.services import annotation_history as history_service
...
@router.get("/history/users/{user_id}", response_model=List[AnnotationHistory])
def get_user_annotation_history(
    user_id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
    skip: int = 0,
...
@router.get("/history/{history_id}", response_model=AnnotationHistory)
def get_history_detail(
    ...
    获取历史记录详情
    """
    history = history_service.get_history_by_id(db=db, history_id=history_id)
    if not history:
        raise HTTPException(
            status_code=404,
            detail="历史记录不存在"
    ...
    # 确保当前用户有权限查看（管理员、相关标注的创建者或审核者）
    annotation = annotation_service.get_annotation(db=db, annotation_id=history.annotation_id)
    if annotation and not current_user.is_superuser and current_user.id != annotation.annotator_id and current_user.id != annotation.reviewer_id:
        raise HTTPException(
            status_code=403,
            detail="无权执行此操作"
        )
